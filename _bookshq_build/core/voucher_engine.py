...
class GSTEngine:
    ...
    def compute_gst(
        ...
        # ── US sales/use tax: a single flat rate, NO state-code split. The
        # India CGST/SGST/IGST logic below runs only for the IN_GST profile.
        ...
        if country.active_profile().tax_system == "US_SALES_TAX":
            ...
            if tax_amount == 0:
                return []
            if is_sales:
                led = self._get_us_tax_ledger(output=True)
                return [VoucherLine(
                    ledger_id=led, cr_amount=tax_amount, is_tax_line=True,
                    tax_type="SALES_TAX", tax_rate=gst_rate_pct,
                    line_narration=f"Sales Tax @{gst_rate_pct}%")]
            led = self._get_us_tax_ledger(output=False)
            return [VoucherLine(
                ledger_id=led, dr_amount=tax_amount, is_tax_line=True,
                tax_type="USE_TAX", tax_rate=gst_rate_pct,
                line_narration=f"Use Tax @{gst_rate_pct}%")]

        # Get party state code  (India GST path)
        row = self.db.execute(
    ...
    def _get_us_tax_ledger(self, output: bool) -> int:
        """US sales-tax (output, a liability) / use-tax (input, an asset) ledger.
        Auto-creates it under the right group on first use, so US companies don't
        need the India CGST/SGST/IGST ledgers seeded."""
        if output:
            name, group, op_type = "Sales Tax Payable", "Duties & Taxes", "Cr"
        else:
            name, group, op_type = "Use Tax Receivable", "Tax Assets", "Dr"
        row = self.db.execute(
            "SELECT id FROM ledgers WHERE company_id=? AND name=?",
            (self.company_id, name)
        ).fetchone()
        ...
        grp = self.db.execute(
            ...
            (self.company_id, group)
        ).fetchone()
        if not grp:
            raise ValueError(
                f"Account group '{group}' not found. Run account seed first.")
        cur = self.db.execute(
            """INSERT INTO ledgers
               ...
               VALUES (?,?,?,0,?,0,0,NULL,NULL,NULL,NULL,0,NULL,NULL,NULL,NULL,NULL)""",
            (self.company_id, grp["id"], name, op_type)
        )
        self.db.commit()
...
class TDSEngine:
    ...
    def should_deduct(self, ledger_id: int, amount: float) -> dict | None:
        ...
        """
        # US 1099 is report-only — never withhold at source. The is_tds_applicable
        # flag instead marks the vendor for the year-end 1099 report
        # (reports_engine.form_1099). Only India (IN_GST) withholds.
        from core import country
        if country.active_profile().tax_system != "IN_GST":
            return None
        row = self.db.execute(
