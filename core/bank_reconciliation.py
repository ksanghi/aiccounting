...
@dataclass
class BalanceCheck:
    """Self-check of a parsed statement: opening + credits − debits == closing.
    A failing tie-out means rows were dropped, duplicated, or misread."""
    opening: float | None
    stated_closing: float | None
    total_credits: float
    total_debits: float
    computed_closing: float | None      # opening + credits − debits
    gap: float | None                   # computed − stated; None when not checkable
    checkable: bool                     # both opening and closing were detected
    balanced: bool                      # checkable and |gap| within tolerance
    txn_count: int
...
class BankReconciler:
    ...
    def __init__(self, db: Database, company_id: int, tree: AccountTree):
        ...
        self.tree = tree
        # Balance tie-out from the most recent import (None until one runs);
        # the UI reads this to show the verification summary.
    ...
    def _persist_statement(
        ...
        # Balance verification BEFORE writing anything. If the statement
        # carries an opening and closing balance and they don't tie out with
        # the parsed lines, refuse the import (so silently-dropped rows can't
        # slip in) unless the user has explicitly chosen to override.
