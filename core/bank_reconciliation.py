...
class BankReconciler:
    ...
    def import_feed_account(
        ...
        *,
        ...
        """
        Ingest a bank-FEED pull (SimpleFIN / Plaid) for an already-linked bank
        ledger. The customer authorised the connection, so there's no file-
        account ownership check. Transactions are deduped against everything
        already imported for this ledger (by reference / FITID / txn id), so
        repeated refreshes never double-import. Returns the new statement_id, or
        0 if there was nothing new.
        """
        if not parse_result.success or not parse_result.lines:
            return 0
        rows = self.db.execute(
            "SELECT bsl.reference FROM bank_statement_lines bsl "
            "JOIN bank_statements bs ON bsl.statement_id = bs.id "
            "WHERE bs.company_id=? AND bs.bank_ledger_id=? "
            "AND COALESCE(bsl.reference,'') != ''",
            (self.company_id, bank_ledger_id),
        ).fetchall()
        seen = {str(r["reference"]) for r in rows}
        fresh = [
            ln for ln in parse_result.lines
            if (not str(ln.get("reference") or "")) or (str(ln["reference"]) not in seen)
        ]
        if not fresh:
            return 0
        lines = self._lines_from_parsed(fresh)
        file_hash = hashlib.sha256(
            ("FEED|" + source_label + "|" + "|".join(
                sorted(str(ln.get("reference") or i) for i, ln in enumerate(fresh))
            )).encode()
        ).hexdigest()
        dup = self.db.execute(
            "SELECT id FROM bank_statements "
        ...
        period_from, period_to = self._resolve_period(
            lines, None, None, parse_result.period_from, parse_result.period_to,
        )
        return self._persist_statement(
            bank_ledger_id=bank_ledger_id,
            ...
            period_from=period_from,
            period_to=period_to,
            statement_opening=parse_result.statement_opening,
            statement_closing=parse_result.statement_closing,
            import_method="FEED",
            imported_by_user_id=user_id,
            raw_meta=json.dumps({
                "source":         "feed",
